...
def update_invoice(data):
    ...
    if selected_currency:
        ...
        for item in invoice_doc.items:
            ...
            if item.rate:
                item.base_rate = flt(
                    item.rate * conversion_rate, item.precision("base_rate")
                )
            if item.amount:
                item.base_amount = flt(
                    item.amount * conversion_rate, item.precision("base_amount")
                )

        # Update payment amounts
        for payment in invoice_doc.payments:
            payment.base_amount = flt(
                payment.amount * conversion_rate, payment.precision("base_amount")
            )
        ...
        invoice_doc.base_in_words = money_in_words(
            invoice_doc.base_rounded_total, company_currency
        )
...
def fetch_exchange_rate_pair(
    from_currency: str, to_currency: str, posting_date: str = None
):
