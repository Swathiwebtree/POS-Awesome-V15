...
def check_opening_shift(user):
    ...
    filters = {'pos_closing_shift': ['in', ['', None]], 'docstatus': 1,
               'status': 'Open'}
    if user_company:
    ...
    open_vouchers = frappe.db.get_all('POS Opening Shift',
            filters=filters, fields=['name', 'pos_profile'],
            order_by='period_start_date desc')
    data = ''
    if len(open_vouchers) > 0:
        data = {}
        data['pos_opening_shift'] = frappe.get_doc('POS Opening Shift',
                open_vouchers[0]['name'])
        update_opening_shift_data(data, open_vouchers[0]['pos_profile'])
    return data

def get_user_company(user):
    company = frappe.db.get_value('User Permission', {'user': user,
                                  'allow': 'Company'}, 'for_value')

    if company:
        return company

    return frappe.defaults.get_user_default('Company', user=user) \
