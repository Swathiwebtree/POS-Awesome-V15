			...
			"@semantic-release/release-notes-generator",
			{
				...
				writerOpts: {
					...
					transform: (commit, context) => {
						...
						commit.shortHash = commit.hash ? commit.hash.substring(0, 7) : null;
						if (commit.references) {
							commit.referencesString = commit.references
								.map((ref) => `#${ref.issue}`)
								.join(", ");
						}
						if (commit.subject) {
							commit.subject = commit.subject.replace(/\(#\d+\)/g, "").trim();
						}
						return commit;
					},
					finalizeContext: (context) => {
						const date = new Date(context.date);
						context.date = dateFormatter.format(date);
						return context;
					},
					mainTemplate:
						"{{> header}}\n\n{{#each commitGroups}}\n### {{title}}\n{{#each commits}}\n{{> commit root=@root}}\n{{/each}}\n_Total: {{commits.length}} changes_\n\n{{/each}}{{#if noteGroups}}\n{{#each noteGroups}}\n### {{title}}\n{{#each notes}}\n- {{text}}\n{{/each}}\n{{/each}}\n{{/if}}\n— Changelog auto-generated by semantic-release. Thanks to all contributors! 🙌\n",
				},
			},
		],
		[
