  ...
  <div class="material-request">
    ...
    <div v-if="selectedMR" class="mr-details">
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Qty</th><th>Unit</th><th>Factor</th><th>Qty Total</th><th>Price</th><th>Amount</th>
          </tr>
        </thead>
        <tbody>
      ...
      <div class="additional-fields">
        ...
        <p><strong>Created By:</strong> {{ selectedMR.created_by || '-' }} on {{ selectedMR.created_on || '-' }}</p>
        <p><strong>Modified By:</strong> {{ selectedMR.modified_by || '-' }} on {{ selectedMR.modified_on || '-' }}</p>
      </div>
    </div>
  </div>
</template>
...
const fetchMaterialRequests = async () => {
  try {
    const res = await axios.get('/api/method/posawesome.posawesome.api.lazer_pos.get_material_request_list', {
      params: {
...
<style scoped>
.material-request { padding: 10px; }
.filters { display: flex; gap: 10px; margin-bottom: 10px; }
table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
th, td { border: 1px solid #ccc; padding: 5px; text-align: left; }
tr:hover { background-color: #f5f5f5; cursor: pointer; }
.mr-details { margin-top: 20px; padding: 10px; border: 1px solid #ddd; background: #fafafa; }
.additional-fields p { margin: 2px 0; }
</style>
