...
const generateReport = async () => {
  ...
  try {
    const response = await axios.get("/api/method/posawesome.posawesome.api.lazer_pos.get_bills_listing_vehicle", {
      params: {
    ...
    reportData.value = response.data.message.bills || [];
  } catch (error) {
    console.error(error);
...
const grandTotal = computed(() => {
  return reportData.value.reduce((sum, row) => sum + (row.net_amount || 0), 0);
});
</script>
