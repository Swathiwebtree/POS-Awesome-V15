...
function addItem() {
  newItems.value.push({
    ac_code: "", account_name: "", description: "", amount: 0,
    division: "", account_id: "", date: "", typ: "",
    trans_no: "", ref_no: "", currency: "", paid_foreign: 0, paid_local: 0
  });
}

// Remove item row
function removeItem(idx) { newItems.value.splice(idx, 1); }
...
async function addPettyCash() {
  try {
    ...
    newItems.value = [];
  } catch (err) { console.error(err); alert("Failed to add Petty Cash"); }
}

// Delete Petty Cash
function deletePettyCash(pc_no) {
  if (confirm("Are you sure you want to delete this entry?")) {
    frappe.delete_doc("Petty Cash", pc_no)
      .then(() => { alert("Deleted!"); fetchPettyCash(); })
      .catch(err => { console.error(err); alert("Failed to delete"); });
  }
}

// Toggle items
function toggleItems(idx) { expanded.value[idx] = !expanded.value[idx]; }
...
.add-form input { margin-right: 8px; margin-bottom: 8px; }
.add-form button { margin-top: 8px; padding: 4px 12px; }
.item-row { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 4px; }
