	...
	<div class="vehicle-service-pos-container">
		...
		<v-row dense>
			<v-col cols="12" md="5">
				...
				<v-text-field label="Scan Barcode" v-model="barcode" @keyup.enter="onBarcodeScan(barcode)" />
				...
				<v-data-table :items="orderItems" :headers="itemHeaders" item-key="item_code" dense>
					...
					<template #item.amount="{ item }"> ₹ {{ (item.qty * item.price).toFixed(2) }} </template>
					<template #item.vat="{ item }">
						₹ {{ (item.qty * item.price * vatRate).toFixed(2) }}
					</template>
			...
			<v-col cols="12" md="7">
				...
				<div class="summary p-3">
					...
					<p>
						<strong>VAT ({{ (vatRate * 100).toFixed(0) }}%):</strong> ₹ {{ vatTotal.toFixed(2) }}
					</p>
...
const itemHeaders = ref([
	{ text: "Service Item", value: "service_item" },
	{ text: "Qty", value: "qty" },
...
// Computed totals
const subtotal = computed(() => orderItems.value.reduce((sum, i) => sum + i.qty * i.price, 0));
const vatTotal = computed(() => subtotal.value * vatRate);
const netTotal = computed(() => subtotal.value + vatTotal.value - workOrderDiscount.value);
...
async function fetchWorkOrderDetails() {
	...
	try {
		const res = await axios.get(
			"/api/method/posawesome.posawesome.api.lazer_pos.get_work_order_details",
			{
				params: { work_order: workOrder.value },
			},
		);
		if (res.data.message) {
...
async function browseItems() {
	try {
		const res = await axios.get("/api/method/posawesome.posawesome.api.get_items");
		if (res.data.message) {
			orderItems.value = res.data.message.map((i) => ({ ...i, qty: 1 }));
		}
	} catch (err) {
		console.error(err);
	}
...
async function onBarcodeScan(code) {
	if (!code) return;
	try {
		const res = await axios.get("/api/method/posawesome.posawesome.api.get_items_from_barcode", {
			params: { barcode: code },
		});
...
// Update line item when qty or price changes
...
async function saveWorkOrder() {
	if (!workOrder.value || !vehicle.value || !customer.value)
		return alert("Fill Work Order, Vehicle & Customer!");
	try {
		await axios.post("/api/method/posawesome.posawesome.api.save_work_order", {
...
async function voidWorkOrder() {
	if (!workOrder.value) return;
	try {
		await axios.post("/api/method/posawesome.posawesome.api.void_order", { order_no: workOrder.value });
		resetOrder();
...
function openPaymentModal() {
	if (!orderItems.value.length) return alert("No items to settle!");
	showPaymentModal.value = true;
}
function applyPayment(method) {
	paidAmount.value = netTotal.value;
	alert(`${method} payment applied for ₹${paidAmount.value}`);
}
...
function completePayment() {
	if (paidAmount.value < netTotal.value) return alert("Payment incomplete!");
	showPaymentModal.value = false;
	resetOrder();
}
...
function openCouponDialog() {
	showLoyaltyCoupon.value = true;
}
async function applyCouponCode(code) {
	alert("Coupon applied: " + code);
	showLoyaltyCoupon.value = false;
}
function applyLoyaltyCustomer(customerObj) {
	customer.value = customerObj.customer_name;
	points.value = customerObj.points;
}
...
function reprintInvoice() {
	alert("Invoice reprinted for " + workOrder.value);
}
