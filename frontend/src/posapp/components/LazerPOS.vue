...
Object.values(transactionSubmodulesMap).forEach((name) => {
  transactionComponents[name] = defineAsyncComponent(() =>
    import(`./modules/transactions/${name}.vue`)
  );
});
...
Object.values(inventorySubmodulesMap).forEach((name) => {
  inventoryComponents[name] = defineAsyncComponent(() =>
    import(`./modules/inventory/${name}.vue`)
  );
});
...
Object.values(reportsSubmodulesMap).forEach((name) => {
  reportsComponents[name] = defineAsyncComponent(() =>
    import(`./modules/reports/${name}.vue`)
  );
});
...
function toggleModule(moduleName) {
  ...
  currentModuleComponent.value = moduleName + "Module";
...
function selectSubmodule(mainModule, submoduleLabel) {
  ...
  currentModuleComponent.value = mainModule + "Module";
}
...
<style scoped>
.LazerPOS { display: flex; min-height: 100vh; }
aside ul li ul li { padding-left: 0.5rem; }
.module-content { background-color: #fff; min-height: 100vh; }
.font-bold.text-green-800 { font-weight: bold; color: #2a7f2e; }
</style>
