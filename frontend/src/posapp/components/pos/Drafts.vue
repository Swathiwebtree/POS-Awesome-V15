...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { title: __("Employee"), value: "custom_service_employee", align: "start", sortable: true, width: "120px" },
  ...
  methods: {
    ...
    async fetchDrafts() {
      ...
      try {
        ...
        drafts = drafts.map((d) => {
          ...
          } else if (d.custom_service_employee && this._employeeNameCache[d.custom_service_employee]) {
    ...
    _normalizeSingle(item) {
      ...
      const posting_time =
        typeof item.posting_time !== "undefined" && item.posting_time !== null ? String(item.posting_time) : "";
      return {
        name: item.name,
