<template>
    ...
    <div class="drafts-content">
      ...
      >
        ...
        <template v-slot:item.posting_time="{ item }">
          <span class="text-caption">{{ item.posting_time ? item.posting_time.split(".")[0] : "" }}</span>
        </template>
  ...
  <v-row justify="center" v-else>
    <v-dialog v-model="draftsDialog" :max-width="isMobileModal ? '100%' : maxWidth" :fullscreen="isMobileModal" :scrollable="true">
      <v-card variant="flat" :color="isDarkTheme ? $vuetify.theme.themes.dark.colors.surface : 'white'">
            ...
            <v-row no-gutters>
              <v-col cols="12" class="pa-1">
                <v-data-table
                  ...
                  :item-class="(item) => isCurrentDraft(item.name) ? 'v-data-table__tr--active' : ''"
                >
                  <template v-slot:column.posting_date="{ column }"><span class="d-none">{{ column.title }}</span></template>
                  <template v-slot:item.posting_date="{ item }"><span class="text-caption">{{ item.posting_date }}</span></template>

                  <template v-slot:column.posting_time="{ column }"><span class="d-none">{{ column.title }}</span></template>
                  <template v-slot:item.posting_time="{ item }"><span class="text-caption">{{ item.posting_time ? item.posting_time.split(".")[0] : "" }}</span></template>

                  <template v-slot:item.name="{ item }">
                    <div class="d-flex align-center">
                      <v-chip v-if="isCurrentDraft(item.name)" color="success" size="x-small" class="mr-2" prepend-icon="mdi-check-circle">
                        {{ __("Current") }}
                      </v-chip>
                      <span class="text-caption font-weight-medium text-primary">{{ item.name }}</span>
                    </div>
                  ...
                  <template v-slot:item.grand_total="{ item }">{{ currencySymbol(item.currency) }}{{ formatCurrency(item.grand_total) }}</template>

                  <template v-slot:bottom>
                    <div class="pa-2 text-caption text-medium-emphasis" v-if="dialog_data.length === 0">{{ __("No draft invoices found.") }}</div>
                    <v-pagination v-else :length="1" :total-visible="3" size="small" class="mt-2"></v-pagination>
                  </template>
                </v-data-table>
              </v-col>
            </v-row>
        ...
        <v-card-actions class="pt-1">
          ...
          <v-btn color="success" variant="flat" @click="submit_dialog" :disabled="selected.length === 0">{{ __("Load Sale") }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
...
export default {
  ...
  computed: {
    isDarkTheme() { return this.$vuetify.theme.current.dark; },
    isMobileModal() { return window.innerWidth < 768; },
  },
  methods: {
    isCurrentDraft(draftName) { return this.$parent?.loaded_draft_name === draftName; },
    ...
    submit_selection() {
      if (this.selected.length > 0) {
        ...
        this.eventBus.emit("show_message", { title: __("Select an invoice to load"), color: "error" });
      }
    },
    ...
    _normalizeSingle(item) {
      ...
      const posting_time = (typeof item.posting_time !== "undefined" && item.posting_time !== null) ? String(item.posting_time) : "";
      return {
        name: item.name,
  ...
  created() {
    ...
    this.eventBus.on("draft_saved", (payload) => {
      ...
      try { this.mergeDraft(payload); } catch (e) { console.warn("[Drafts] mergeDraft failed", e); }
    });
    ...
    this.eventBus.on("invoice_saved_successfully", (payload) => {
      console.log("[Drafts] invoice_saved_successfully received -> merging if payload provided", payload);
...
.drafts-wrapper { display: flex; flex-direction: column; height: 100%; width: 100%; background: white; }
.drafts-content { flex: 1; overflow-y: auto; overflow-x: hidden; background-color: #fafafa; }
.drafts-table { background-color: white; }
:deep(.v-data-table-row td), :deep(.v-data-table-row th) { padding: 8px 12px !important; }
:deep(.v-data-table) { font-size: 0.875rem; }
:deep(.v-data-table__wrapper) { overflow-y: auto; max-height: 100%; }
:deep(.v-data-table__tr--active) { background-color: rgba(76, 175, 80, 0.08) !important; }
:deep(.v-data-table__tr--active:hover) { background-color: rgba(76, 175, 80, 0.12) !important; }
.drafts-footer { flex-shrink: 0; padding: 12px; background: white; border-top: 2px solid #e0e0e0; z-index: 100; }
.load-draft-btn { text-transform: uppercase; font-weight: 600; letter-spacing: 0.5px; }
.drafts-content::-webkit-scrollbar { width: 6px; }
.drafts-content::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 3px; }
.drafts-content::-webkit-scrollbar-thumb { background: #999; border-radius: 3px; }
.drafts-content::-webkit-scrollbar-thumb:hover { background: #666; }
@media (max-width: 768px) {
  :deep(.v-data-table-row td), :deep(.v-data-table-row th) { padding: 6px 8px !important; font-size: 0.8rem; }
  .drafts-footer { padding: 8px; }
}
</style>
