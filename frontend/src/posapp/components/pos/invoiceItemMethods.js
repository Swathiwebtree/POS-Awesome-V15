...
export default {
	...
	async new_order(data = {}) {
		...
		if (!data.name && !data.is_return) {
			...
			console.log("[new_order] Fresh order state:", {
				...
				contact_mobile: this.contact_mobile,
				custom_vehicle_no: this.custom_vehicle_no,
				custom_vehicle_make: this.custom_vehicle_make,
			});
		} else {
			console.log(
	...
	get_invoice_doc() {
		...
		doc.redeem_loyalty_points = doc.redeemed_loyalty_points;
		doc.loyalty_amount = Number(
			doc.loyalty_amount ?? this.invoice_doc?.loyalty_amount ?? this.loyalty_redemption_amount ?? 0,
		);
	...
	async show_payment() {
		try {
			...
			console.log("[show_payment] Setting invoice totals:", {
				total: invoice_doc.total,
				net_total: invoice_doc.net_total,
				grand_total: invoice_doc.grand_total,
				rounded_total: invoice_doc.rounded_total,
				backend_grand_total: invoice_doc.grand_total,
			});
			...
			// This MUST be called AFTER setting the correct totals
