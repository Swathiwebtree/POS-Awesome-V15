			...
			<v-autocomplete v-else-if="vehicles.length > 1" ref="vehicleDropdown"
				...
				<template #item="{ props, item }">
					<v-list-item v-bind="props">
						...
						<v-list-item-subtitle v-if="item.raw.customer_name">Customer: {{ item.raw.customer_name
							}}</v-list-item-subtitle>
						<v-list-item-subtitle v-if="item.raw.mobile_no">Mobile: {{ item.raw.mobile_no
							}}</v-list-item-subtitle>
					</v-list-item>
				</template>
			</v-autocomplete>
