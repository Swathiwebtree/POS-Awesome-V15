...
export default {
	props: {
		...
		additional_discount: [Number, String], // Updated type to include String for input fields
		additional_discount_percentage: [Number, String], // Updated type to include String for input fields
		...
		isNumber: Function,
		// NEW PROP: Pass the selected customer ID from the parent component
	...
	computed: {
		isDarkTheme() {
			// Assuming $theme is globally available in the Vuetify context
			return this.$vuetify?.theme?.global?.current?.dark;
		},
		hide_qty_decimals() {
			try {
				// IMPORTANT: In a real Frappe/ERPNext environment, localStorage usage is common
	...
	methods: {
		// Removed unused onCustomerSelect(customer) method
		...
		apply_additional_discount() {
			// This is a basic calculation mock. The real discount calculation
			// is usually handled in the parent component that manages the whole invoice state.
			// We just ensure the update events are triggered.
		...
		// UPDATED: Use the selectedCustomerId prop
		async handleLoyaltyPoints() {
			const customerId = this.selectedCustomerId;

			if (!customerId) {
				// Using frappe.__ for translation
				frappe.show_alert({ message: frappe.__("Please select a customer first"), indicator: "red" });
				return;
			}

			this.loyaltyLoading = true;
			try {
				// The frappe.call is correctly set up based on the provided backend code
				...
				frappe.show_alert({
					// Use frappe.__ and string interpolation
					message: frappe.__(`Loyalty Points: ${points}`),
					indicator: "green",
				});
			} catch (err) {
				console.error("Failed to fetch loyalty points:", err);
				frappe.show_alert({
					message: frappe.__("Error fetching loyalty points. Check console for details."),
					indicator: "red",
				});
			} finally {
