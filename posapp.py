...
def update_invoice(data):
    ...
    if data.get('is_return') and data.get('return_against'):
        ...
        original_invoice = frappe.get_doc("Sales Invoice", data.get('return_against'))
        
        # Validate each return item
        for item in data.get('items', []):
            # Find matching item in original invoice
            original_item = next((i for i in original_invoice.items if i.item_code == item.get('item_code')), None)
            
            if not original_item:
                frappe.throw(f"Item {item.get('item_code')} not found in original invoice")
                
            # Validate rate
            if abs(flt(original_item.rate) - flt(item.get('rate'))) > 0.01:
                frappe.throw(f"Rate mismatch for item {item.get('item_code')}")
                
            # Validate quantity
            if abs(flt(item.get('qty'))) > flt(original_item.qty):
                frappe.throw(f"Return quantity cannot be greater than original quantity for item {item.get('item_code')}")
                
            # Validate warehouse if specified
            if item.get('warehouse') and item.get('warehouse') != original_item.warehouse:
                frappe.throw(f"Warehouse mismatch for item {item.get('item_code')}")
                
            # Validate batch if specified
            if item.get('batch_no') and item.get('batch_no') != original_item.batch_no:
                frappe.throw(f"Batch mismatch for item {item.get('item_code')}")
